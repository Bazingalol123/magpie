...
## 2026-08-28 — some Deno tests grep `src/App.jsx`'s literal source text, not just its behavior, which breaks on component-only file moves

While splitting `src/App.jsx` (Build Guide checkpoint 72), several tests in `tests/` turned out to be "grep tests": they call `Deno.readTextFile` on `src/App.jsx` and assert on exact substrings of its source rather than exercising any behavior. Five assert on a literal `"function <ComponentName>"` declaration (`tests/pairing-lifecycle-ui.test.ts` for `PairingManagementDialog`; `tests/onboarding-flow-wiring.test.ts` for `CaptureSourceOffer`; `tests/redesign-ui-parity.test.ts` for `WatchDialog`, `ComparisonPanel`, `CollectionDeleteControl`) — these break the instant that component moves to its own file, regardless of whether its behavior changed at all. Several more pin other literal JSX/effect snippets (exact class names, exact prop-wiring strings, an exact `useEffect` dependency-array line) that happen to live wherever the moved code used to sit.
