...
> Produced 2026-08-16 by the issue #47 pre-beta source-of-truth audit and
> updated with the direct Base44 release evidence from 2026-08-24. This
...
The original audit did not have production Base44 access. Its older
"deployed" claims are backed by GitHub Actions run IDs. On 2026-08-24, the
linked app was also released directly with documented Base44 CLI commands:
all ten entity schemas synced, all 24 Functions were present remotely, and
the site returned `200` on both public domains. Seven anonymous Function
calls returned their intended `401` boundary. No signed-in production
pairing-management or hosted two-owner walkthrough was performed, so those
journeys remain deployed but not live-verified.
...
| Redesigned Nest / Library / Signals / Search workspace | deployed, anonymously live-smoked | Site deployed 2026-08-24 and both public domains returned `200`. Authenticated desktop/mobile behavior was verified locally; no signed-in production walkthrough was performed. |
| Saved searches, short route undo, audited field correction | deployed, auth boundary live-verified | Required additive entity fields and all three Functions were deployed 2026-08-24. Anonymous calls reached each Function and returned `401`; signed-in mutation paths were not exercised in production. |
| Connected Browsers list/revoke/reconnect lifecycle | deployed, auth boundary live-verified | Pairing schemas and four changed Functions were deployed 2026-08-24. Anonymous calls returned the expected `401`; local two-owner and token-rejection checks pass, but the hosted two-owner gate remains open. |
...
| Area | Status | Basis |
|---|---|---|
| Chrome Side Panel | source current, released in v0.3.1 | `extension/manifest.json` uses `side_panel.default_path` and the `sidePanel` permission. The v0.3.1 GitHub Release exists, but it predates issue #61's credential-clearing and browser-ID changes. |
| Extension pairing-lifecycle artifact | source-only / locally-verified | The issue #61 source was pushed to GitHub, but the Base44 deploy does not publish a Chrome artifact. A new extension release has not been cut. |
...
24 Functions exist in `base44/functions/`. All 24 `entry.ts` files pass
`deno check` (2026-08-24). Per-function deployment/verification status:
...
| `list-extension-pairings`, `revoke-extension-pairing`, `revoke-all-extension-pairings` | deployed, auth boundary live-verified | Targeted deploy 2026-08-24. Anonymous calls returned `401`; sanitized output, idempotent revoke, token rejection, and two-owner isolation are locally verified only. |
| `correct-record-field`, `create-saved-search`, `undo-routing-resolution` | deployed, auth boundary live-verified | Targeted deploy 2026-08-24 after the site inventory exposed these as required redesign dependencies. Anonymous calls returned `401`; signed-in production mutations were not performed. |

## Testing and CI

| Claim | Status | Evidence |
|---|---|---|
| `deno test` count | locally-verified: **255/255** | Re-run 2026-08-24 before the direct release. |
| `deno check` across all backend entry points | locally-verified: **24/24** | Re-run 2026-08-24 before the direct release. |
...
- No undo for permanent deletions or dismissal. A narrow 30-second undo exists
  only for a just-created Collection route and refuses once dependent history
...
- No production Base44 dashboard sign-in during the 2026-08-24 release, so no
  live click-through of the redesigned workspace or pairing lifecycle was
  performed.
- No `npx base44 logs` access, so no production error-rate or log-based
  verification.
- Entity schemas were directly synced with `npx base44 entities push` on
  2026-08-24. Agent configuration was not changed or re-pushed in that
  release.
