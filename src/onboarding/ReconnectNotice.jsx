import { Key, LoaderCircle, PlugZap } from "lucide-react";
  ...
  return (
    <section className="onboarding-panel onboarding-reconnect" role="region" aria-label="Reconnect the extension">
      <div className="onboarding-step">
        <span className="onboarding-step-icon"><PlugZap size={15} /></span>
        <div>
        ...
        <button type="button" className="onboarding-cta" onClick={onPair} disabled={isPairing}>
          {isPairing ? <LoaderCircle className="spin" size={15} /> : <Key size={15} />} Reconnect extension
        </button>
      </div>
    </section>
  );
