...
// HTML. There is no Side Panel button or keyboard shortcut for selection
// mode in production — only the right-click context menu — and Chromium's
// native context-menu items are not automatable via Playwright/CDP (see the
// doc comment on triggerCaptureTab in helpers/capture.ts). This test sets a
...
// sidepanel.js's own "Save page" button already uses in production, which
...
test("selection capture reads the real page selection with no raw_html, and dedupes on retry", async ({
  ...
  const first = await triggerCaptureTab(sidePanel, tabId, "selection");
  ...
  const retry = await triggerCaptureTab(sidePanel, tabId, "selection");
